    ...
    for (iterationsSoFar = 0; iterationsSoFar < iterationsPerCalculation; iterationsSoFar++) {
      ...
      if (searchNode.y > 0) {
        checkAdjacentNode(
          ...
          0,
          -1,
          STRAIGHT_COST * costMap[collisionGrid[searchNode.y - 1][searchNode.x]]
        );
      ...
      if (searchNode.x < collisionGrid[0].length - 1) {
        checkAdjacentNode(
          ...
          1,
          0,
          STRAIGHT_COST * costMap[collisionGrid[searchNode.y][searchNode.x + 1]]
        );
        if (instances[0].isDoneCalculating === true) {
          instances.shift();
          continue;
      ...
      if (diagonalsEnabled) {
        if (searchNode.x > 0 && searchNode.y > 0) {
          checkAdjacentNode(
            ...
            -1,
            -1,
            DIAGONAL_COST * costMap[collisionGrid[searchNode.y - 1][searchNode.x - 1]]
          );
          if (instances[0].isDoneCalculating === true) {
            instances.shift();
            continue;
...
Phaser.Plugin.PathFinderPlugin.prototype.setGrid = function(grid, walkables, iterationsPerCount) {
  ...
  for (var i = 0; i < grid.length; i++) {
    ...
    for (var j = 0; j < grid[i].length; j++) {
      if (grid[i][j]) this._grid[i][j] = grid[i][j].index;
      else this._grid[i][j] = 0;
    }
  }
